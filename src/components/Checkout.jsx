...
import { addToCart, setCartItems } from '../features/products/productSlice'

function Checkout() {
  const dispatch = useDispatch()
  // const { id } = useParams()
  const cartItemSelected = useSelector((state) => state.product.cartItems)
  ...
  const handleMinusClick = (index) => {
    if (product[index].quantity > 1) {
      ...
      updatedProducts[index] = {
        ...updatedProducts[index],
        setQuantity: updatedProducts[index].quantity - 1,
      }
      setProduct(updatedProducts)

      // Update the corresponding item in the cartItems array
      const updatedCartItems = [...cartItemSelected]
  ...
  useEffect(() => {
    // Create an array of product objects from the cartItemSelected array
    const products = cartItemSelected.map((item) => ({
