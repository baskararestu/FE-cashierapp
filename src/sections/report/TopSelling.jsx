...
const DailySales = () => {
  ...
  const fetchDailySales = async () => {
    try {
      const response = await axios.get(
        'http://localhost:3000/transaction/top-selling-products',
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('user_token')}`,
          },
  ...
  return (
    <div className="top-selling">
      <h3 className="text-lg uppercase font-semibold py-6">
      ...
      <div className="flex flex-row gap-5">
        <div className="filter sm:w-1/3 w-full">
          <div className="flex flex-col justify-start border p-2 gap-3">
            <label htmlFor="filter" className="w-[120px]">
              Filter Category
            </label>
            <div className="gap-2 flex flex-row flex-wrap">
              <button
                ...
                className={`${
                  filter.category === null ? '!border-orange-300 border-2' : ''
                } border border-gray-300 uppercase rounded-md px-2 py-1`}
              >
                ALL
              </button>
